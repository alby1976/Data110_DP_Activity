...
| **Classification bias** | Rule-based interpretation of category, proposed-use, description, or district text can misclassify residential type or rezoning relevance. | Use a version-controlled lookup; retain original fields; assign `Review` to ambiguous values; manually inspect a seeded reproducible sample; report unmatched records; test broader and narrower definitions. | Free text and multi-use applications may remain ambiguous. `RezoningRelevant` is analytical, not an official City label. |
