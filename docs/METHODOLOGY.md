...
`add_processing_features` is an implemented pure filter that preserves every
record and its original index. `ProcessingDays` is a nullable signed integer
computed from calendar dates, retaining negative evidence. Only rows with
`HasValidProcessingDays` should enter duration summaries. Validity requires
usable dates and a duration at least `analysis.processing_time.minimum_days`
(default zero), which is bound by the CLI. `HasNegativeProcessingDays` identifies
negative durations; `ProcessingDateMissing` and `ProcessingDateInvalid` keep
absent values separate from malformed values, including cleaner invalid flags.

`IsPending` is an analytical proxy for a valid application with a genuinely
missing decision, not an official permit status. Invalid decisions do not count
as pending. Optional `observation_end` lets callers censor decisions after a
known inclusive snapshot horizon and exclude applications after that horizon.
`IsAfterObservationEnd` flags either date beyond it. `FollowUpDays` measures a
...
`ProcessingAnalysis` implements these summaries for records with
`IncludeResidential=true` and `HasValidProcessingDays=true`. It reports median,
mean, linearly interpolated quartiles, and IQR in days. Empty valid cohorts have
null statistics; valid zero-day decisions are retained. Configured empty periods
remain in the summary. Type breakdowns are generated when the type field exists;
community breakdowns require an explicit source column.

The strategy reports residential, valid, ineligible, pending, and right-censored
counts, with optional negative-duration, missing-date, invalid-date, and
after-horizon counts. Missing audit flags produce null counts rather than assumed
zeros. Reasons can overlap. `InvalidCount` means all residential records excluded
from duration statistics, including censoring and minimum-duration exclusions;
it does not mean every such record is erroneous. Nonresidential exclusions are
counted separately in `processing_period_totals`.

Flags are supplied by the feature stage; the analysis does not infer an observation
date or recompute eligibility. It rejects valid rows with missing/negative durations
or contradictory pending, censoring, or date-quality flags. Input records are
preserved. These are descriptive summaries of observed valid intervals, not
survival estimates; observation-horizon wiring and minimum-follow-up policy remain
outstanding before final cohort comparisons are accepted.
